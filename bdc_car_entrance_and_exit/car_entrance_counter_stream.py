...
class CarEntranceCounterStream:
    ...
    def process_stream(self, save_output=False, output_path="output_entrance_stream.mp4", display=True):
        """
        ...
        if display:
            cv2.namedWindow('Car Entrance Counter - LIVE', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('Car Entrance Counter - LIVE', 1280, 720)
...
def main():
    # Configuration
    STREAM_URL = "rtsp://apt:apt123@desktop-bdc:8554/entrance"
    MODEL_PATH = "yolo12m.pt"
    OUTPUT_PATH = "output_entrance_stream.mp4"
